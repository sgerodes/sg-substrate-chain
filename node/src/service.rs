...
use polkadot_sdk::sc_service::{
    Configuration, TaskManager, WarpSyncConfig, error::Error as ServiceError,
};
...
pub fn new_partial(config: &Configuration) -> Result<Service, ServiceError> {
    let telemetry = config
        ...
        .filter(|x| !x.is_empty())
        .map(
            |endpoints| -> Result<_, polkadot_sdk::sc_telemetry::Error> {
    ...
    let import_queue = polkadot_sdk::sc_consensus_aura::import_queue::<AuraPair, _, _, _, _, _>(
        ImportQueueParams {
            ...
            create_inherent_data_providers: move |parent_hash, _| {
                ...
                async move {
                    let slot_duration =
                        polkadot_sdk::sc_consensus_aura::standalone::slot_duration_at(
                            &*cidp_client,
                            parent_hash,
                        )?;
                    let timestamp =
...
pub fn new_full<
    N: polkadot_sdk::sc_network::NetworkBackend<
            Block,
...
>(
    ...
    let warp_sync = Arc::new(
        polkadot_sdk::sc_consensus_grandpa::warp_proof::NetworkProvider::new(
            backend.clone(),
    ...
    if role.is_authority() {
        ...
        let aura = polkadot_sdk::sc_consensus_aura::start_aura::<
            AuraPair,
            _,
            _,
            _,
